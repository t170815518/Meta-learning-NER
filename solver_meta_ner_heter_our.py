...
from utils.functions import sample_sorted_alignment_minibatch_from_numpy_with_domain_y
...
class MetaNERHeter_baseline(object):
    ...
    def init_network(self):
        ...
        for name in self.train_domains:
            TAG_TO_NUMBER, RE_PATTENS = get_TAG_and_pattens('single_' + name)
            self.RE_PATTENS[name] = RE_PATTENS
            self.TAG_TO_NUNBER[name] = TAG_TO_NUMBER
            cur_total_entity_type = len(TAG_TO_NUMBER)
            self.all_crf[name] = MyCRF(self.params.rnn_direction_no * self.params.encoder_rnn_hidden,
                                       cur_total_entity_type, self.device).to(self.device)

            train_x_word = pickle.load(
                    open(os.path.join(self.params.foldpath + '/%s' % name, 'train_x_word_mapping'), 'rb'))
            train_x_char = pickle.load(
                    open(os.path.join(self.params.foldpath + '/%s' % name, 'train_x_char_mapping'), 'rb'))
            ...
            dev_x_word = pickle.load(
                    open(os.path.join(self.params.foldpath + '/%s' % name, 'dev_x_word_mapping'), 'rb'))
            dev_x_char = pickle.load(
                    open(os.path.join(self.params.foldpath + '/%s' % name, 'dev_x_char_mapping'), 'rb'))
    ...
    def save_params(self, params, savepath):
        ...
        with open(os.path.join(savepath, 'a_params_tracking.txt'), 'a') as f:
            f.write('training size: %d, dev size: %d ' % (
                    sum(len(self.train_data[cur_name][0]) for cur_name in self.train_domains),
                    sum(len(self.dev_data[cur_name][0]) for cur_name in self.train_domains)) + '\n')
    ...
    def check_performance(self, check_data):
        ...
        for cur_domain in self.train_domains:
            ...
            for lp in range(need_loop):
                ...
                inputTensors = sample_sorted_alignment_minibatch_from_numpy(select_data, self.batch_size,
                                                                            sampleFlag=False, device=self.device)
    ...
    def train(self):
        ...
        cur_save_path = os.path.join(self.params.save_path,
                                     'heter_base_%d_sources_' % self.params.no_train_domain + date_time)
        os.makedirs(cur_save_path)
        self.save_params(self.params, cur_save_path)

        self.opt_all_exc_crf = optim.Adam([{'params': self.feature_extractor.wordrep.word_embed.parameters(),
                                            'lr': self.params.word_embedding_lr, 'weight_decay': 0},
        ...
        for iter_id in range(total_iter):
            cur_epoch = int(np.ceil(iter_id / num_each_epoch + 0.00000001))
            cur_iter = iter_id % num_each_epoch
            print("epoch:%d/%d,iteration:%d/%d" % (cur_epoch, self.params.no_epoch, cur_iter, num_each_epoch))

            for cur_domain in self.train_domains:
                inputTensors = sample_sorted_alignment_minibatch_from_numpy(self.train_data[cur_domain],
                                                                            self.batch_size,
...
class MetaNER_Heterogeneous_Solver(MetaNERHeter_baseline):
    def __init__(self, params, DEVICE):
        super(MetaNER_Heterogeneous_Solver, self).__init__(params, DEVICE)
    ...
    def train(self):
        ...
        self.opt_feature = optim.Adam([{'params': self.feature_extractor.wordrep.word_embed.parameters(),
                                        'lr': self.params.word_embedding_lr, 'weight_decay': 0},
                                       {'params': self.feature_extractor.base_params}],
                                      lr=self.params.lr, weight_decay=self.params.weight_decay)
        ...
        for iter in range(total_iter):
            ...
            for cur_domain in meta_train_domains:
                inputTensors = sample_sorted_alignment_minibatch_from_numpy_with_domain_y(
                        self.train_domains_index[cur_domain], self.train_data[cur_domain],
                        self.batch_size,
                        sampleFlag=True, device=self.device)
            ...
            temp_crf_weights = {}
            for name in self.train_domains:
                temp_crf_weights[name] = OrderedDict(
                        (name, param) for (name, param) in self.all_crf[name].named_parameters())
            ...
            grads_f_old = torch.autograd.grad(meta_train_loss_main, temp_f_weights.values(), retain_graph=True)  # we
            # do not take second order , create_graph=True
            temp_f_weights_old = OrderedDict((name, param - self.params.alpha * grad) for ((name, param), grad) in
                                             zip(temp_f_weights.items(), grads_f_old))

            # temp_f_weights_new makes error prediction
            grads_f_new = torch.autograd.grad(meta_train_loss_adv, temp_f_weights.values(), retain_graph=True)  # ,
            # create_graph=True
            temp_f_weights_new = None
            if self.params.theta_update == 'all':
                temp_f_weights_new = OrderedDict((name, param - self.params.alpha * grad) for ((name, param), grad) in
                                                 zip(temp_f_weights_old.items(), grads_f_new))
            if self.params.theta_update == 'ori':
                temp_f_weights_new = OrderedDict((name, param - self.params.alpha * grad) for ((name, param), grad) in
                                                 zip(temp_f_weights.items(), grads_f_new))
            ...
            for name in meta_train_domains:
                grads_crf_final[name] = torch.autograd.grad(meta_train_loss_main, temp_crf_weights[name].values(),
                                                            retain_graph=True)

            for name in meta_val_domains:
                grads_crf_final[name] = torch.autograd.grad(meta_loss_val, temp_crf_weights[name].values(),
                                                            retain_graph=True)
            ...
            clip_grad_norm_(
                    list(self.feature_extractor.parameters()) + temp + list(self.domain_classifier.parameters()), 5.0)
            ...
            if iter % check_point_iter_no == 0 and iter != 0:
                ...
                SeqPRF = self.check_performance(self.evaluate_train_data)
                track_performance = ['train', cur_epoch, iter] + SeqPRF
                with open(os.path.join(cur_save_path, 'a_performance_tracking.txt'), 'a') as f:
                    f.write('\t'.join(map(str, track_performance)) + '\n')
                ...
                SeqPRF = self.check_performance(self.dev_data)
                track_performance = ['dev', cur_epoch, iter] + SeqPRF
                with open(os.path.join(cur_save_path, 'a_performance_tracking.txt'), 'a') as f:
                    f.write('\t'.join(map(str, track_performance)) + '\n')
            ...
            if self.params.lr_decay and (cur_epoch != pre_epoch):
                scheduler_f.step()
                for name in self.train_domains:
