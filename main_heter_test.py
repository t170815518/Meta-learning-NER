...
def check_with_fine_tuning(taskname,ratio, batch_size, best_feature, best_crf, feature_extractor, crf_layer, train_check_data, dev_check_data, test_check_data, device):
    ...
    for iter in range(total_iter):
        ...
        inputTensors = sample_sorted_alignment_minibatch_from_numpy(train_check_data, batch_size, sampleFlag=True, device=device)
        ...
        if iter % check_point_iter_no == 0 and iter != 0:
            ...
            Precison, Recall, F1 = check_without_fine_tuning(batch_size, feature_extractor, crf_layer, test_check_data, device)

            #print('dev F1:',F1)

            if best_dev_F1 < F1:
                best_dev_F1 = F1
                best_feature.load_state_dict(copy.deepcopy(feature_extractor.state_dict()))  #pickle.loads(pickle.dumps(feature_extractor))   #deep copy?
...
if __name__ == '__main__':
    ...
    for cur_ratio in data_ratio:
        ...
        if FINE_TUNING == False:
            ...
            best_crf = MyCRF(params.rnn_direction_no * params.encoder_rnn_hidden, cur_total_entity_type, DEVICE).to(DEVICE)
            P, R, F1  = check_with_fine_tuning(target_name, data_per, b_size, best_f, best_crf, feature_extractor, crf_layer, train_check_data, dev_check_data, test_check_data, DEVICE)
